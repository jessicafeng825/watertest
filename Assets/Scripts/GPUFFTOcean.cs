...
public class GPUFFTOcean : MonoBehaviour
{
    ...
    public float MeshLength = 250;	//网格长度
    ...
    public float frameCount = 128; //帧率
    ...
    void Update()
    {
        ...
        //if(count < 128){
        //    SaveTex(DisplaceRT,filefordisplacement);
        //    SaveTex(NormalRT,filefornormal);
        //    SaveTex(BubblesRT,fileforbubble);
       // }
          //count++;
        if(count >= frameCount){
            Debug.Log("Finish");
        ...
        private void SaveTex(RenderTexture renderTexture,string file){
     ...
     IEnumerator SaveTexture(){
        ...
        ComputeOceanValue();
        string filefordisplacement = string.Format("D:/unity project/watertest/Assets/Resources/Textures/res/Displace/{0}.png",count);
        string filefornormal = string.Format("D:/unity project/watertest/Assets/Resources/Textures/res/Normal/{0}.png",count);
        string fileforbubble = string.Format("D:/unity project/watertest/Assets/Resources/Textures/res/Bubbles/{0}.png",count);
        SaveTex(DisplaceRT,filefordisplacement);
        SaveTex(NormalRT,filefornormal);
        SaveTex(BubblesRT,fileforbubble);
    ...
    private void InitializeCSvalue()
    {
        ...
        //设置ComputerShader数据
        OceanCS.SetInt("N", fftSize);//1024
        OceanCS.SetFloat("OceanLength", MeshLength);//512


        //生成高斯随机数
        OceanCS.SetTexture(kernelComputeGaussianRandom, "GaussianRandomRT", GaussianRandomRT);
        OceanCS.Dispatch(kernelComputeGaussianRandom, fftSize / 8, fftSize / 8, 1);//ComputeGaussianRandom
    ...
    private void ComputeOceanValue()
    {
        ...
        WindAndSeed.w = Random.Range(1, 10f);//风设置随机种子
        ...
        OceanCS.Dispatch(kernelCreateHeightSpectrum, fftSize / 8, fftSize / 8, 1);//CreateHeightSpectrum
        ...
        OceanCS.Dispatch(kernelCreateDisplaceSpectrum, fftSize / 8, fftSize / 8, 1);//CreateDisplaceSpectrum
        ...
        OceanCS.Dispatch(kernelTextureGenerationDisplace, fftSize / 8, fftSize / 8, 1);//TextureGenerationDisplace
        ...
        OceanCS.Dispatch(kernelTextureGenerationNormalBubbles, fftSize / 8, fftSize / 8, 1);//TextureGenerationNormalBubbles
