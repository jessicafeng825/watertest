Shader "Unlit/waterforcaustic"
{
    Properties
    {
        ...
        _strength("casutic strength", Range(0, 5)) = 1.0
    ...
    SubShader
    {
        ...
        sampler2D _Displace;
        sampler2D _Normal;
        float4 _Displace_ST,_Normal_ST;
        ...
        v2f vertWater(a2v v)
        {
            ...
            float3 normal = UnityObjectToWorldNormal(tex2Dlod(_Normal, float4(o.uv3, 0, 0)).rgb);
        ...
        { 
            ...
            fixed3 worldNormal = normalize(i.worldNormal);
            fixed3 worldLightDir = normalize(UnityWorldSpaceLightDir(i.worldPos));
            fixed3 diffuse = _strength *_LightColor0.rgb * _Color.rgb * tex2D(_MainTex, i.uv).rgb * max(0, dot(worldNormal, worldLightDir));
